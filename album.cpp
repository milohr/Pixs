...
void Album::createAlbum(const QString &artist, const QString &album, const QString &imagePath,const PixsUtils::ALbumSizeHint &widgetSize, const int &widgetRadius, const bool &isDraggable)
{
    switch (widgetSize)
    {
    case PixsUtils::BIG_ALBUM:
        this->size=PixsUtils::getWidgetSizeHint(PixsUtils::BIG_ALBUM_FACTOR,PixsUtils::BIG_ALBUM);
        this->subSize=PixsUtils::BIG_ALBUM_FACTOR_SUBWIDGET;
        break;
    case PixsUtils::MEDIUM_ALBUM:
        this->size=PixsUtils::getWidgetSizeHint(PixsUtils::MEDIUM_ALBUM_FACTOR,PixsUtils::MEDIUM_ALBUM);
        this->subSize=PixsUtils::MEDIUM_ALBUM_FACTOR_SUBWIDGET;
        break;
    case PixsUtils::SMALL_ALBUM:
        this->size=PixsUtils::getWidgetSizeHint(PixsUtils::SMALL_ALBUM_FACTOR,PixsUtils::SMALL_ALBUM);
        this->subSize=PixsUtils::SMALL_ALBUM_FACTOR_SUBWIDGET;
        break;
    }
...
void Album::artIt_action()
{
    ...
    QString path = QFileDialog::getOpenFileName(this, tr("Select Music Files"),PixsUtils::CachePath, tr("Image Files (*.png *.jpg *.bmp)"));
    if(!path.isEmpty())
    {
        putPixmap(path);
